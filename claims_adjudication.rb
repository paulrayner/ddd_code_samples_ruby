...
class ClaimsAdjudication
  def adjudicate(contract, new_claim)
    if limit_of_liability(contract) > new_claim.amount &&
       in_effect_for?(contract, new_claim.failure_date)
  ...
  def in_effect_for?(contract, failure_date)
    failure_date >= contract.effective_date &&
    failure_date <= contract.expiration_date &&
